function [dRate_score,dRate_vec,dISI_score, dISI_vec, nAP, Score_vec]  = Test_dRate(spike_time_states,Inj_vec, plot_flag )
...
% John: 09132021
    ...
    for i=1: n_sweeps
        ...
        if sum(Spike_ind_1)>=6 && sum(Spike_ind_1)<=15
...
%             Ramp_vec(i)=b(2);   % Units are Hz/s
            ...
            if abs(n_spike_1-10)<clst_drate
                clst_drate=abs(n_spike_1-10);
                clst_ind=i;
                nAP=n_spike_1;
            end
        end
    ...
    if ~isempty(clst_ind)
        ...
        Spike_ind_1= spike_time>200 ;  % Get all the spike
        n_spike_1=sum(Spike_ind_1);
        if n_spike_1>1
            spike_time_stable=spike_time(Spike_ind_1);
            ISI_vec_1=spike_time_stable(2:end)-spike_time_stable(1:end-1);
    ...
    Score_vec=[CV_ISI_score,ISIratio_score,dISI3_score,dISI2_score,dInitft_score];
end
